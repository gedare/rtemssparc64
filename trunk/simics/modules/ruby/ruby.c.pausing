...
cycles_t ruby_operate(conf_object_t *obj, conf_object_t *space,
  ...
  cycles_t cycles = 0;
  if(!isPaused){
  	cycles = mh_memorytracer_possible_cache_miss(mem_op);
  	//printf("Ruby churning returned: %d\n",cycles);
  }
...
//		"If paused, no memory operations are passed to ruby.");
