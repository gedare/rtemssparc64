...
)
{
  ...
  if (index_base >= information->maximum ) {
    ...
    block_size = block_count *
           (sizeof(void *) + sizeof(uint32_t) + sizeof(uint32_t)/* <-this is a quick fix for 64-bit alignment */ + sizeof(Objects_Name *)) +
    ...
    local_table = (Objects_Control **) _Addresses_Add_offset(
        inactive_per_block, block_count * 2 /* <-this is a quick fix for 64-bit alignment */ * sizeof(uint32_t) );
